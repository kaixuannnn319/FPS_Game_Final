...
public class FloatingBoss : MeleeGuard
{
    ...
    protected override Vector3 GetChaseDestination()
    {
        ...
        if (dist < minComfortDistance)
        {
            ...
            away.Normalize();
            return ClampToArena(player.position + away * retreatToDistance); // anchored to player, not current position
        }
        ...
        return ClampToArena(player.position + dirFromPlayer * (MaxAttackRange() * 0.8f));
    }
